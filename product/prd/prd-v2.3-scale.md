...
v2.3 is the **adoption flywheel**. It aligns with the YC "Make Something Agents Want" thesis: agents should not have to click through human UIs, wait for manual setup, or reverse-engineer brittle dashboards. ASAP should make APIs, MCP servers, CLIs, and SDKs discoverable, sign-up friendly, and usable programmatically.

Each of the three core features removes a specific friction:
...
### 1.3 Adoption Thesis A + B

The selected strategy is **Developer Adoption + API Onboarding**:

- **A: Developer Adoption First** — meet TypeScript and agent-framework developers where they already build: Vercel AI SDK, OpenAI Agents SDK, Google ADK, LangGraph, Mastra, Microsoft Agent Framework, and similar ecosystems.
- **B: API Onboarding First** — turn existing OpenAPI specs into ASAP-compatible capabilities so API providers can become agent-first without rebuilding their backend.

Auto-Registration remains important, but it should be gated and measured as an adoption accelerant rather than treated as the core value proposition of v2.3.0.
...
| Homepage activation | Landing page and docs links updated for every backend/protocol adoption release | P0 |
...
### 4.6 Adapter Discovery Backlog (P0 research, P1/P2 delivery in v2.3.x)

v2.3.0 ships the core SDK and reference adapters. The v2.3.x patch train should test which adjacent ecosystems create the strongest adoption pull.

| Ecosystem | Why it matters | Candidate v2.3.x shape |
|-----------|----------------|-------------------------|
| OpenAI Agents SDK | Lowest-friction GPT-centric agent stack; strong fit for hosted tools and handoffs | Adapter + guide: ASAP capabilities as tools and remote agents |
| Google ADK | Strong GCP/Gemini adoption and A2A adjacency | Bridge guide + example agent exposing ASAP manifest |
| LangGraph | Production stateful agent graphs; complements existing LangChain integration | Recipe: ASAP providers as graph nodes/tools |
| Mastra | TypeScript-first agent framework for Next.js and Node teams | `@asap-protocol/client` integration example and template |
| Microsoft Agent Framework / Semantic Kernel | Enterprise and Azure/.NET reach | Interop spike, then adapter if enterprise demand appears |
| Haystack / Letta / AG2 | Search, memory, and multi-agent niches | Research spikes; promote only if user requests appear |
| n8n / Activepieces / Zapier-style automation | Workflow users can expose existing tools to agents | Connector/piece prototypes after SDK adoption is measurable |

Delivery rule: every promoted adapter must include a minimal example, Compliance Harness validation where applicable, documentation, and homepage/site copy pointing users to the guide.
...
| Adapter demand validated | 3+ external users or projects asking for the same framework adapter | Per v2.3.x patch |
| Homepage-to-docs conversion | Trackable CTA from site to GitHub docs/examples | Per release |
...
- **v2.3.0 sprint index**: [tasks-v2.3.0-adoption-multiplier.md](../../engineering/tasks/v2.3.0/tasks-v2.3.0-adoption-multiplier.md)
...
### Patch train (optional local PRDs)

Tactical follow-ups live under `product/prd/private/` when present locally (ignored by default): Adapter Lab I/II, Distribution Loop — see §7.1.
