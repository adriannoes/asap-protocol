...
**Depends on**: v2.4.1 shipped; `MCPServer` + `verify_agent_jwt` in tree
...
- [ ] 1.1 Write design lock ADR
  ...
  - **What**: Document chosen hook: **wrapper** `ProtectedMCPServer` delegating to inner `MCPServer` OR **monkey-patch** `_handle_tools_call` via `protect_server`. Record decision on `_meta` field vs raw dict access. Note: unprotected servers must remain default.
...
- [ ] 3.2 Define `MCPAuthConfig` dataclass
  ...
  - **What**: Fields per PRD §6.1 (`host_store`, `agent_store`, `tool_capability_map`, `public_tools`, `enforce_grants`, `hide_unauthorized_tools`, `jwt_extractor`, `manifest_url`)
  - **Why**: Central configuration for S1–S2
  - **Verify**: mypy clean on new module

- [ ] 3.3 Stub `protect_server` raising `NotImplementedError`
...
- [ ] `asap.adapters.mcp` importable; `MCPAuthConfig` typed
