...
class TestSQLiteMeteringStore:
    ...
    async def test_query_pagination(
        ...
        page1 = await sqlite_metering_store.query("a1", start, end, limit=2, offset=0)
        assert len(page1) == 2
        assert page1[0].task_id == "t0"
        assert page1[1].task_id == "t1"

        # Page 2: limit=2, offset=2
        page2 = await sqlite_metering_store.query("a1", start, end, limit=2, offset=2)
        assert len(page2) == 2
        assert page2[0].task_id == "t2"
        assert page2[1].task_id == "t3"

        # Page 3: limit=2, offset=4
        page3 = await sqlite_metering_store.query("a1", start, end, limit=2, offset=4)
        assert len(page3) == 1
