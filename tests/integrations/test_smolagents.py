...
@pytest.mark.asyncio
async def test_smolagents_tool_init_inside_running_loop_defers_resolve() -> None:
    """Inside a running event loop, __init__ must not asyncio.run(resolve); forward resolves lazily."""
    task_response = TaskResponse(
        task_id="task-1",
        status="completed",
    ...
    with p_get as mock_get, p_verify as mock_verify, p_revoked as mock_revoked, p_http:
        ...
        with patch("asap.client.market.ASAPClient", return_value=mock_transport):
            ...
            with patch.object(client, "resolve", mock_resolve):
                tool = SmolAgentsAsapTool(TEST_URN, client=client)
                assert tool._resolved is None
                mock_resolve.assert_not_awaited()
                out = await tool._invoke_async({"input": {"q": "async-host"}})
