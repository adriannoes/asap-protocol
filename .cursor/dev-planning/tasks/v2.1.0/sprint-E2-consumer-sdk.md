...
- `src/asap/client/cache.py` — Registry cache (TTL from ASAP_REGISTRY_CACHE_TTL; get_registry, invalidate)
- `src/asap/client/market.py` — MarketClient.resolve, ResolvedAgent.run
- `src/asap/client/http_client.py` — 429 retry (Retry-After / exponential backoff), get_with_429_retry
- `docs/raw-fetch.md` — Raw Fetch doc (registry.json, revoked_agents.json; curl/fetch; schema; non-Python)
- `tests/client/test_market.py`, `tests/client/test_cache.py` — Client tests
...
- [x] `MarketClient.resolve(urn)` returns ResolvedAgent; fetches registry (cached 5 min), manifest, validates trust, checks revocation
- [x] `agent.run(payload)` orchestrates HTTP/WebSocket handshake; returns result dict
- [x] Registry cache TTL configurable via `ASAP_REGISTRY_CACHE_TTL` (default 300)
- [x] HTTP 429 handled with exponential backoff; honors `Retry-After`
- [x] Bearer token passthrough via `MarketClient(auth_token=...)` or `agent.run(..., auth_token=...)`
- [x] DOC-001: Raw Fetch path documented for non-Python consumers

---

## Task 2.1: Create client cache module

- [x] **2.1.1** Implement registry cache with TTL
...
- [x] **2.2.1** Implement MarketClient.resolve flow
...
- [x] **2.2.2** Implement ResolvedAgent.run flow
...
- [x] **2.2.3** Write tests for MarketClient (resolve + run + signature + revoked)
...
- [x] **2.3.2** Write test for 429 retry
...
- [x] **2.5.2** Add link from README or docs index
...
- [x] **2.6.1** Exports and public API
