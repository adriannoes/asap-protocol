...
- **Web App**: Next.js frontend (reads from Lite Registry)
- **Verified Badge**: Stripe payment integration

> [!NOTE]
> FastAPI Registry API Backend and backend services are deferred to v2.1+. See [deferred-backlog.md](./deferred-backlog.md).
...
4.  **Incremental Roadmap**: Features like the Compliance Harness (v1.2) and Delegation Tokens (v1.3) serve *both* the Protocol and the Marketplace. A hard split creates unnecessary ambiguity about where code belongs.
...
### Phase 2: v2.0.0 (Lean Marketplace)
When we begin building the Marketplace Web App:
1.  Create an `apps/web/` directory for the Next.js frontend.
2.  The Web App reads from Lite Registry (`registry.json` on GitHub Pages) — no backend API needed.
3.  Treat the Web App as a separate internal "workspace" with its own `package.json`.

### Phase 3: v2.1+ (Re-assessment)
We will consider extracting `apps/web/` to a separate repository ONLY when:
- The Web App (Next.js) requires a completely independent deployment, versioning, and release cycle from the Protocol SDK.
- The Registry API Backend (FastAPI, v2.1) introduces a backend that also needs its own deployment cycle.
