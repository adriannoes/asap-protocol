...
> See [ADR-17](../../product/decision-records/03-security.md) for the decision record.
...
- Example: `ASAP_AUTH_CUSTOM_CLAIM=https://github.com/adriannoes/asap-protocol/agent_id`
- **Future namespace**: `https://asap-protocol.com/agent_id` will be the canonical claim URI when the domain is available; until then, the GitHub URL is used.

**Code**

When creating the app, pass `OAuth2Config(..., manifest_id=manifest.id)`. The middleware uses `manifest_id` for this check. You can override the claim key with `OAuth2Config(..., custom_claim="https://github.com/adriannoes/asap-protocol/agent_id")`.
...
**Fail-closed (v2.3.x+):** When `manifest_id` is configured and neither the custom claim nor an allowlist entry matches, the server returns **403** (identity mismatch). Configure `ASAP_AUTH_CUSTOM_CLAIM` or `ASAP_AUTH_SUBJECT_MAP` before enabling OAuth2 in production.

### Issuer and audience validation

When configured, the middleware rejects tokens whose `iss` or `aud` do not match:

| Environment variable | Purpose |
|:---------------------|:--------|
| `ASAP_AUTH_ISSUER` | Expected JWT `iss` (also used for OIDC discovery when JWKS URI is derived) |
| `ASAP_AUTH_AUDIENCE` | Expected JWT `aud`; comma-separated for multiple values |

Pass the same values via `OAuth2Config(expected_issuer=..., expected_audience=...)` in code. When unset, issuer/audience checks are skipped (backward compatible).
...
3. Add the agent URN to `idToken` and/or `accessToken` under the claim name that matches `ASAP_AUTH_CUSTOM_CLAIM` (default: `https://github.com/adriannoes/asap-protocol/agent_id`).
...
**Actions**: Create an Action that runs at “Login / Post Login”, and set `event.idToken['https://github.com/adriannoes/asap-protocol/agent_id'] = 'urn:asap:agent:my-bot'` (and same for `event.accessToken` if you use access tokens for ASAP).  
...
3. Set **Token Claim Name** to the same value as `ASAP_AUTH_CUSTOM_CLAIM` (default: `https://github.com/adriannoes/asap-protocol/agent_id`).
...
- **Claims mapping**: In the app manifest, define `optionalClaims` or use claims mapping policies to emit `https://github.com/adriannoes/asap-protocol/agent_id` (or your chosen URI) with value `urn:asap:agent:my-bot`.
...
- [ADR-17: Trust Model and Identity Binding](../../product/decision-records/03-security.md)
- [Security Guide](../security.md) — general ASAP security (rate limits, nonce, HTTPS, etc.)
- [Transport](https://github.com/asap-protocol/asap-protocol/blob/main/docs/transport.md) — OAuth2 middleware and `create_app` usage
